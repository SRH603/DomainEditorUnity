...
namespace RuntimeCurveEditor
{
    ...
    {
        static string VERSION = "2.3.1";
        static string VERSION_KEY = "VERSION_KEY";
        ...
        public static void SaveData(string configName, Object obj, CurveWindow curveWindow, List<CurveForm> curveFormList,
            ...
            data += ":" + zoomLevel + ":" + ((gridRect.x - entireRect.x) / gridRect.width) + ":" + ((gridRect.y - entireRect.y) / gridRect.height);
            ...
            foreach (CurveForm curveForm in curveFormList) {
                ...
                if (curve != null) {
                    foreach (Keyframe key in curve.keys) {
                        data += ":" + key.inTangent + ":" + key.inWeight + ":" + key.outTangent + ":" + key.outWeight + ":" + key.time + ":" + key.value + ":" + (int)key.weightedMode;
                    }
                ...
                if (curve2 != null) {
                    ...
                    data += ":" + curve2.length;
                    //TODO take care, below is duplicated code
                ...
                foreach (var c in new AnimationCurve[]{ curve, curve2})
                {
                    if (c != null)
                    {
                        foreach (ContextMenu contextMenu in dictCurvesContextMenus[c])
                        {
                            data += ":" + contextMenu;
                        }
                        data += ":";
                ...
                if (field.FieldType == typeof(AnimationCurve)) {
                    ...
                    if (curveForm == null) {
                        ...
                        if (curveForm == null) {//this curve is not added to the curve editor, however we store it too
                            ...
                            data += ":" + curve.length;
                            foreach (Keyframe key in curve.keys)
                            {//TODO, take care, duplicated code below
                                data += ":" + key.inTangent + ":" + key.inWeight + ":" + key.outTangent + ":" + key.outWeight + ":" + key.time + ":" + key.value + ":" + (int)key.weightedMode;
                            }
                            data += ":";//empty string as the end for a curve data
                        }
        ...
        public static void LoadData(string configName, Object obj, CurveWindow curveWindow, CurveLines curveLines, List<CurveForm> curveFormList,
            ...
            curveWindow.GetComponent<ZoomBehaviour>().Level = float.Parse(elements[i++]);

            curveLines.zoomRatioX = float.Parse(elements[i++]);
            curveLines.zoomRatioY = float.Parse(elements[i++]);
            ...
            for (int k = 0; k < curveFormCount; ++k) {
                ...
                if (length > 0) {
                    ...
                    for (int j = 0; j < length; ++j) {
                        Keyframe key = new Keyframe(float.Parse(elements[i + 4]), float.Parse(elements[i + 5]), float.Parse(elements[i]),
                                                    float.Parse(elements[i + 2]), float.Parse(elements[i + 1]), float.Parse(elements[i + 3]));
                        key.weightedMode = (WeightedMode)int.Parse(elements[i + 6]);
                        curve.AddKey(key);
                        i += 7;
                    }
