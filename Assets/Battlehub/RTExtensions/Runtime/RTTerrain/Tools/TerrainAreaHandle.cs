...
namespace Battlehub.RTTerrain
{
    public class TerrainAreaHandle : BaseHandle
    {
        ...
        private Vector3[] m_areaResizerPositions;
        public Vector3[] AreaResizerPositions
        {
            get { return m_areaResizerPositions; }
            set 
            {
                m_areaResizerPositions = value;
                if (SystemInfo.supportsGeometryShaders)
                {
                    m_areaResizerMesh.vertices = m_areaResizerPositions;
                }
                else
                {
                    Vector3[] vertices = new Vector3[m_areaResizerPositions.Length * 4];
                    for (int i = 0; i < m_areaResizerPositions.Length; ++i)
                    {
                        vertices[i * 4] = m_areaResizerPositions[i];
                        vertices[i * 4 + 1] = m_areaResizerPositions[i];
                        vertices[i * 4 + 2] = m_areaResizerPositions[i];
                        vertices[i * 4 + 3] = m_areaResizerPositions[i];
                    }
                    m_areaResizerMesh.vertices = vertices;
                }
        ...
        protected override void RefreshCommandBuffer(IRTECamera camera)
        {
            ...
            if(SelectedAxis == RuntimeHandleAxis.Custom && m_areaResizerIndex >= 0)
            {
                ...
                {
                    colors[m_areaResizerIndex * 4] = Appearance.Colors.SelectionColor;
                    colors[m_areaResizerIndex * 4 + 1] = Appearance.Colors.SelectionColor;
                    colors[m_areaResizerIndex * 4 + 2] = Appearance.Colors.SelectionColor;
                    colors[m_areaResizerIndex * 4 + 3] = Appearance.Colors.SelectionColor;
                }
            }
        ...
        private void BuildPointsMesh(Mesh target, int pointsCount)
        {
            ...
            m_areaResizerPositions = vertices;
        }
