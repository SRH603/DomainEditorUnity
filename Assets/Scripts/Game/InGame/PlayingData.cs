...
public class PlayingData : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (generateLevel.NotesNum > 0)
        {
            ...
            OptimalScore = 1000000 * (OptimalNum / (double)generateLevel.NotesNum);
            PerfectScore = 1000000 * ((EarlyPerfectNum + LatePerfectNum) / (double)generateLevel.NotesNum);
            GoodScore = 1000000 * ((EarlyGoodNum + LateGoodNum) / (double)generateLevel.NotesNum) * 0.5;
            //ComboScore = 50000 / (double)generateLevel.NotesNum * MaxCombo;
            Score = Mathf.RoundToInt((float)(OptimalScore + PerfectScore + GoodScore));
